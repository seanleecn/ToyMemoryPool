...
//const size_t FREE_LIST_SIZE = MAX_SIZE / 8; // 数组总共 8*1024 个链表，每个链表的元素是[8byte,16byte,...,64*1024byte]
const size_t FREE_LIST_SIZE = 184; // 在UnitTest中测试了取整结果就184种情况，也就是184个链表就够了
const size_t MAX_PAGES = 129;      // 最大能管理4kb*128 = 512kb的内存
...
public:
    ...
    static size_t ListIndex(size_t size) {
        ...
        // 每个区间有多少个链
        static int group_array[4] = {16, 56, 56, 56}; // 在UnitTest中测试了只有184种结果
        if (size <= 128) {
...
#ifdef _WIN32
    void *ptr = VirtualAlloc(nullptr, num_page * (1 << PAGE_SHITF), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
#else
    // brk mmap等
#endif
    if (ptr == nullptr)
